...
<nav
    class="fixed top-0 left-0 right-0 z-[1000] transition-all duration-300 {scrolled
        ...
        <div class="flex items-center justify-between h-20">
            ...
            <div class="hidden md:flex items-center space-x-8">
                ...
                <button
                    ...
                    class="nav-link">{t($locale, "nav.projects")}</button
                >
                <button
                    on:click={() => scrollToSection("experience")}
                    class="nav-link">{t($locale, "nav.experience")}</button
                >
                <button
                    on:click={() => scrollToSection("contact")}
                    class="nav-link">{t($locale, "nav.contact")}</button
                >
                <button
...
<!-- Mobile Menu Drawer (outside <nav> to avoid backdrop-filter containing block issue) -->
{#if mobileMenuOpen}
    <!-- Backdrop Overlay -->
    <div
        class="fixed inset-0 bg-black/60 backdrop-blur-sm z-[999] md:hidden"
    ...
    <!-- Side Drawer -->
    <div
        class="fixed top-0 right-0 h-full w-[80%] max-w-[340px] bg-white/95 dark:bg-zinc-950/95 backdrop-blur-xl border-l border-gray-200/50 dark:border-white/10 shadow-2xl z-[1001] p-6 flex flex-col justify-between md:hidden"
        ...
        <div class="flex flex-col h-full justify-between overflow-y-auto pr-1">
            ...
            >
                ...
                >
                    ...
                    <button
                        on:click={toggleDarkMode}
                        class="relative inline-flex h-6 w-11 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none {darkMode
                            ? 'bg-blue-600'
