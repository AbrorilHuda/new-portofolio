<script lang="ts">
  ...
  const experiences = [
    ...
    },
    {
      ...
      period: "2022 - now",
      description_id:
        "Saya mahasiswa Universitas Madura yang aktif belajar melalui kontribusi langsung, seperti mengelola komunitas developer (MaduraDev), membangun komunitas belajar bersama Dicoding (DCN UNIRA), dan membantu sesama sebagai asisten praktikum. Saya senang terlibat dalam proyek kolaboratif yang bermanfaat.",
      description_en:
        "I am a Madura University student who actively learns through direct contributions, such as managing a developer community (MaduraDev), building a learning community with Dicoding (DCN UNIRA), and helping peers as a practicum assistant. I enjoy being involved in beneficial collaborative projects.",
      type: "education",
    },
  ];
...
<section
  id="experience"
  class="py-20 md:py-32 px-6 lg:px-8 bg-gray-50 dark:bg-zinc-950/20 border-y border-gray-200/30 dark:border-white/5"
>
  <div class="max-w-5xl mx-auto">
    <div class="text-center mb-16">
      <h2
        class="font-display text-3xl sm:text-4xl md:text-5xl font-bold tracking-tight mb-4"
      >
        {t($locale, "experience.titlePrefix")} &
        <span
          class="bg-linear-to-r from-blue-600 to-purple-600 bg-clip-text text-transparent"
          >{t($locale, "experience.titleHighlight")}</span
        >
      </h2>
      <p
        class="text-gray-500 dark:text-gray-400 text-base sm:text-lg max-w-2xl mx-auto"
      >
    ...
    <div class="relative">
      <!-- Vertical Line -->
      <div
        class="absolute left-8 md:left-1/2 top-0 bottom-0 w-0.5 bg-linear-to-b from-blue-600 to-purple-600"
      ></div>
      ...
      <div class="space-y-12">
        {#each experiences as exp, i}
          <div
            class="relative flex flex-col md:flex-row gap-8 {i % 2 === 0
              ? 'md:flex-row'
              : 'md:flex-row-reverse'}"
          >
            <!-- Timeline Dot -->
            <div
              class="absolute left-8 md:left-1/2 w-4 h-4 bg-linear-to-r from-blue-600 to-purple-600 rounded-full transform -translate-x-1/2 shadow-lg"
            >
              <div
                class="absolute inset-0 bg-linear-to-r from-blue-600 to-purple-600 rounded-full animate-ping opacity-75"
              ></div>
            </div>

            <!-- Content Card -->
            <div
              class="md:w-1/2 {i % 2 === 0
                ? 'md:pr-12 md:text-right'
                : 'md:pl-12'}"
            >
              <div
                class="ml-16 md:ml-0 bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-lg hover:shadow-xl transition-shadow"
              >
                <div
                  class="flex items-center gap-2 mb-2 {i % 2 === 0
                    ? 'md:justify-end'
                    : ''}"
                >
                  {#if exp.type === "work"}
