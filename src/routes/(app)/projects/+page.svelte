...
<svelte:head>
  <title
    >{$locale === "en" ? "My Projects" : "Proyek Saya"} - Moh.AbrorilHuda</title
  >
  <meta
    name="description"
...
<div class="min-h-screen pt-32 pb-24 px-6 lg:px-8 max-w-7xl mx-auto">
  ...
  <div class="text-center mb-12 space-y-4">
    <h1
      class="font-display text-4xl sm:text-5xl md:text-6xl font-extrabold tracking-tight text-gray-900 dark:text-white"
    >
      {t($locale, "projects.titlePrefix")}
      <span
        ...
        >{t($locale, "projects.titleHighlight")}</span
      >
    </h1>

    <p
      class="text-base sm:text-lg text-gray-500 dark:text-gray-400 max-w-2xl mx-auto leading-relaxed"
    >
      {$locale === "en"
  ...
  <div class="mb-12 space-y-6">
    ...
    <div class="max-w-xl mx-auto relative">
      ...
      <input
        ...
        placeholder={$locale === "en"
          ? "Search project by name or tech..."
          : "Cari proyek berdasarkan nama atau teknologi..."}
    ...
    {#if availableTechs.length > 1}
      <div
        class="flex flex-wrap items-center justify-center gap-2 max-w-4xl mx-auto"
      >
  ...
  {#if filteredProjects.length === 0}
    <div
      class="py-20 text-center bg-gray-50/50 dark:bg-zinc-900/30 rounded-3xl border border-gray-200/50 dark:border-white/5 max-w-xl mx-auto space-y-3"
    >
      <svg
