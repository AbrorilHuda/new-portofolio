      ...
      {#if !previewMode}
        ...
        >
          ...
          <div class="flex justify-end space-x-4">
            <a 
              href="/admin/blogs"
              class="px-6 py-3 bg-gray-200 dark:bg-gray-700 rounded-lg hover:bg-gray-300 dark:hover:bg-gray-600 font-medium transition-colors"
            >
              Batal
            </a>
            <button
              type="submit"
              disabled={submitting}
              class="px-6 py-3 bg-blue-600 text-white rounded-lg hover:bg-blue-700 font-medium transition-all flex items-center justify-center gap-2 disabled:opacity-60 disabled:cursor-not-allowed"
            >
              {#if submitting}
                ...
                <span>{published ? 'Mempublikasikan...' : 'Menyimpan...'}</span>
              {:else}
                <span>{published ? 'Publish' : 'Simpan sebagai Draft'}</span>
              {/if}
            </button>
          </div>
        </form>
